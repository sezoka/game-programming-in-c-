...
draw_bg_sprite_component :: proc(bg: BG_Sprite_Component, c: ^Component, renderer: ^sdl.Renderer) {
	for tex in bg.bg_textures {
		...
		r.x = i32(c.owner.position.x - f32(r.w) / 2 + tex.offset.x)
		r.y = i32(c.owner.position.y - f32(r.h) / 2 + tex.offset.y)
...
update_bg_sprite_component :: proc(bg: ^BG_Sprite_Component, s: ^Component, delta: f32) {
	for tex in &bg.bg_textures {
		tex.offset.x += bg.scroll_speed * delta
		if tex.offset.x < -bg.screen_size.x {
			tex.offset.x = f32((len(bg.bg_textures) - 1)) * bg.screen_size.x - 1
		}
	}
}
