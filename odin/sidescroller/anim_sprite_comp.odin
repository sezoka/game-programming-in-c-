...
update_anim_sprite_component :: proc(s: ^Anim_Sprite_Component, c: ^Component, delta: f32) {
	if len(s.anim_textures) < 0 do return

	s.curr_frame += s.anim_fps * delta

	for f32(len(s.anim_textures)) <= s.curr_frame {
		s.curr_frame -= f32(len(s.anim_textures))
	}
