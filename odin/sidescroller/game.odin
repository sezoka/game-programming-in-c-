...
process_input :: proc(g: ^Game) {
	...
	for actor in &g.actors {
		...
		case Ship_Actor:
			process_keyboard_ship(state, &inner, actor)
		}
	}
}
...
load_data :: proc(g: ^Game) {
	ship_actor := create_ship_actor(g)
	ship_actor.position = {
		x = 100,
		y = HEIGHT / 2,
	}
	ship_actor.scale = 1.5
	// ship := ship_actor.variant.(Ship_Actor)

	// background
	bg_actor := create_actor(g)
	bg_actor.position = {WIDTH / 2, HEIGHT / 2}

	bg_component := create_bg_sprite_component(bg_actor)
	...
	// closer background
	bg_component = create_bg_sprite_component(bg_actor, 50)
	bg = &bg_component.variant.(BG_Sprite_Component)
	bg.screen_size = {WIDTH, HEIGHT}
